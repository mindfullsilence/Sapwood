 ...
 */
class Text_link extends Component {
   ...
   */
  public function validate_object($valid) {
    // href is required
    if(empty($this->object['data']['href'])) return false;
    return true;
  }
  ...
  public function format_data($data = array()) {
    ...
    $href = '';
    if(is_numeric($data['href'])) {
      $p = \Timber\Timber::get_post($data['href']);
      $href = $p->link;
      $data['text'] = $data['text'] ?: $p->post_title;
    } else {
      $href = $data['href'];
      $data['text'] = $data['text'] ?: $href;
    }

    $data['attributes']['href'] = $href;
    $data['attributes']['class'] = "c-{$this->name}";
